...
- **exe 文件图标定为本项目原创图形**（`assets/icon-app.ico`，胖鲸，MIT，7 种尺寸内嵌）。
  它不含 DeepSeek 的任何标识，因此**构建出的二进制可以自由分发**；托盘图标不受影响，
  仍是运行时从使用者本机 DSH 读取的官方鲸鱼。
- `tools/make-app-icon.js`：图标生成器（维护者改图时运行）。
- 自检与 CI 新增断言：构建产物必须内嵌 `assets/icon-app.ico`（漏掉会静默退回 .NET 默认图标）。

### 变更

- **构建不再需要 Node.js / sharp**：exe 图标是仓库内的静态资源，托盘图标运行时渲染。
  构建链只剩 `csc` 一条。
- 托盘图标降级链由三级简化为两级（去掉 exe 同目录 `dsh.ico`）：运行时渲染 → exe 内嵌图标。
- `-SkipIcon` 语义改为「编译但不内嵌图标」。
